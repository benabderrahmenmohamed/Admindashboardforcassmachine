...
import { addItem, removeLine, setQty, totals } from '../cart';
import { addToCartProblem, filterProducts, quantityProblem } from '../selling';
import type { Cart, CheckoutPayment, PaymentMethod, Product } from '../types';
...
/** Products, search, barcode entry and the cart, for a terminal with an open session. */
export function SellingScreen({ cart, onCartChange, onCheckout, isRecording }: SellingScreenProps) {
  ...
  const addToCart = (product: Product) => {
    ...
    onCartChange(addItem(cart, product));
  ...
  const updateQuantity = (productId: string, change: number) => {
    ...
    onCartChange(setQty(cart, productId, newQuantity));
  };

  const removeFromCart = (productId: string) => {
    onCartChange(removeLine(cart, productId));
  };

  const handleCompletePayment = async (payment: CheckoutPayment) => {
  ...
  return (
    <>
      ...
      <CheckoutDialog
        ...
        onConfirm={(payment) => void handleCompletePayment(payment)}
