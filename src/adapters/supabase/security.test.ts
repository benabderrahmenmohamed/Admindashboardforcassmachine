 ...
 *                              may make
 * and signs in the seeded accounts of shop A (admin@demo.local, cashier@demo.local) and shop B
 * (other-admin@demo.local, other-cashier@demo.local).
 */
...
async function signedIn(email: string, password: string): Promise<SupabaseDatabaseClient> {
...
describe.runIf(contractBackendIs('supabase'))('Supabase row-level security and ledger', () => {
  ...
  beforeAll(async () => {
    ...
    adminA = await signedIn('admin@demo.local', 'demo-admin-2026');
    cashierA = await signedIn('cashier@demo.local', 'demo-cashier-2026');
    adminB = await signedIn('other-admin@demo.local', 'other-admin-2026');
    cashierB = await signedIn('other-cashier@demo.local', 'other-cashier-2026');
  ...
  it('shows a cashier of shop B no row of shop A in any table', async () => {
    for (const [table, rowsOf] of Object.entries(SHOP_TABLES)) {
      const seen = await rowsOf(cashierB, SHOP_A);
